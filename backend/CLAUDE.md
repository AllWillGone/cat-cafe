...
| DELETE /api/user/me | 仅有关联点赞/评论，或订单均为已支付/已完成 | 删除本人点赞、评论、可注销订单和用户记录，注销成功 200 |
| DELETE /api/user/me | 存在未支付、制作中、已取消等不可注销订单 | 400 |
...
- 新增 `backend/cleanup.py`，集中清理 `comment.targetType + targetId` 和 `likes.likeType + objectId` 软引用。
- 删除评论时同步删除该评论收到的点赞。
- 删除猫咪时同步删除猫咪点赞、猫咪下评论、以及评论点赞。
- 删除商品时同步删除商品点赞、商品下评论、以及评论点赞；若因历史订单不能硬删，仍按原规则下架并执行清理。
- 注销用户或管理员删除用户时，删除用户评论前先清理别人给这些评论点过的赞。
- 已复查删除入口：订单删除、取消点赞不产生被删除目标的点赞残留。
